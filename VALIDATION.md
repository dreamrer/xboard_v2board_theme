...
## v1.1.0（移植 Aurora 功能 + V2board 支持）
...
### 对齐 Aurora 功能面（v1.2.0）

按从小到大的顺序补齐了 Aurora 有而 HeroRui 缺的四项，每项都核对过两个面板的真实接口：
...
逐项对比 Aurora 的接口面（46 个）、配置项（30 个）与实现细节后补上的：

- **加密中间件的伪装 404 没有任何线索**。中间件把密钥不符、时钟偏差超窗、入口前缀不符、伪装后缀不符四种成因一律回同一个 404，Aurora 会把这四条打到控制台，HeroRui 只显示「请求失败 (404)」。已补齐，并断言四条成因都在提示里。
- **知识库文章里的嵌入视频被整个剥掉**。HeroRui 原来全站只有一档净化配置，`iframe` 一律剥除；教程里嵌视频是这类站点的普遍做法。改为两档：套餐描述 / 公告仍然全剥（应为纯排版），知识库放行 `iframe` 及其必要属性。两档都照样剥 `script`、`on*` 与 `javascript:`。顺带核实 Aurora 的 STRICT 名单里 `iframe / object / embed / base / link / meta` 本来就不在 DOMPurify 默认允许列表里，属于冗余；`ping` / `formaction` 也已被默认剥除。
- **8 本语言词典里有 7 本永远用不上**。`getLocale()` 硬编码默认 `zh-CN`，英文访客首次打开看到中文。Aurora 只有 3 种语言但按 `navigator.language` 自动选。已补上浏览器语言识别（港澳台按繁体、无对应词典回落 `zh-CN`），用户手动选过就不再自动改。这个值同时作为 `Content-Language` 发给后端，影响其返回文案与邮件语言。
...
- 真实的加密中间件实例（Go 版 encrypt-middleware）对接：本地只验证了协议往返，未与真实中间件握手。
- Xboard「每日签到」插件的真实返回值（本地无该插件，按 Aurora 的实现推导契约）；V2board 内置签到的真实响应。
