...
export function AuthPage({route}){
 ...
 //
 // 登录前必须先读到这份配置（要不要验证码、邮箱验证），读不到时登录按钮是灰的。
 // 所以失败时要说清楚为什么点不了，并给一个重试按钮 —— 否则用户只能刷新整页。
 const [configError,setConfigError]=useState(''),[configTry,retryConfig]=useState(0),[configLoading,setConfigLoading]=useState(true);
 useEffect(()=>{let active=true;setConfigLoading(true);setConfigError('');api('/guest/comm/config').then(c=>{if(active){setConfig(guestConfig(c));setConfigLoading(false)}}).catch(e=>{if(!active)return;setConfigError(e.message);setConfigLoading(false);
  if(e.network)console.warn('[登录] 读取站点配置失败。分离部署请确认 config.js 的 server_url 可以访问：直接在浏览器打开 <server_url>/api/v1/guest/comm/config 应返回 JSON；Cloudflare 522 表示源站连不上。')});return()=>{active=false}},[configTry]);
 ...
 return <div className="auth-page"><header className="auth-top"><a className="brand" href="#/login"><span className="brand-icon">h.</span><span>{settings.title||'HeroRui'}</span></a><Select aria-label="Language" value={locale} onChange={setLocale} options={languageOptions} variant="borderless" suffixIcon={<Globe size={16}/>}/></header><div className="auth-stage"><div className="auth-note"><span className="eyebrow">YOUR WORLD, UNRESTRICTED.</span><h1>{register?'Make it\nyours.':forget?'A fresh\nstart.':'Hello,\nagain.'}</h1><p>{settings.description||'连接，自有新意。'}</p><span className="auth-edition">01 — ACCOUNT ACCESS</span></div><section className="auth-form"><span className="mini-index">{register?'02':forget?'03':'01'} / {t('用户')}</span><h2>{t(register?'注册':forget?'重置密码':'登入')}</h2>{configError&&<Alert className="config-error" type="error" showIcon title={t('无法连接到面板，暂时不能登录')} description={configError} action={<Action variant="secondary" size="sm" loading={configLoading} onClick={()=>retryConfig(n=>n+1)}>{t('重试')}</Action>}/>}{error&&<Alert type="error" title={error}/>}<Form form={form} layout="vertical" onFinish={submit} requiredMark={false} initialValues={{invite_code:params.get('code')||'',suffix:(register||forget)&&whitelist[0]?'@'+whitelist[0]:undefined}}>
