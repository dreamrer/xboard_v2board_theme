...
export function Profile(){const t=useT(),{user,config,reload}=useAccount(),{modal,message}=AntApp.useApp(),{busy,run}=useAction(),[form]=Form.useForm(),[bot,setBot]=useState(null);const password=v=>run(async()=>{await post('/user/changePassword',{old_password:v.old_password,new_password:v.new_password});form.resetFields()},t('密码修改成功'));
 /**
  ...
  *
  * 剪贴板：Safari / iOS 只允许在用户点击的同步调用栈里写剪贴板，等请求回来再写
  * 就已经不算用户操作了，会被拒绝。所以点击时**立刻**调用 clipboard.write，
  * 把「还在路上的链接」作为 Promise 交给 ClipboardItem，浏览器会等它落地再写入。
  * 不支持这种写法的浏览器退回普通复制；仍然失败就弹窗展示链接，让用户再点一次复制。
  */
 const [quick,setQuick]=useState('');
 const quickLogin=()=>run(async()=>{
  const pending=post('/user/getQuickLoginUrl').then(url=>{if(!url||typeof url!=='string')throw Error(t('生成失败，请稍后再试'));return url});
  let copied=false;
  if(window.isSecureContext&&navigator.clipboard?.write&&typeof ClipboardItem==='function'){
   try{await navigator.clipboard.write([new ClipboardItem({'text/plain':pending.then(u=>new Blob([u],{type:'text/plain'}))})]);copied=true}catch{}
  }
  const url=await pending;
  if(!copied){try{await copyText(url);copied=true}catch{}}
  if(copied)message.success(t('链接已复制，60 秒内有效'));else setQuick(url);
 });
